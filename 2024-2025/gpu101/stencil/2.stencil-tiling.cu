...
__global__ void stencil_kernel_tiling_gpu(const float *__restrict__ in, float *__restrict__ out,
                                          const int N) {
...
int main() {
	...
	long long elapsedCPU = end - start;
	...
	CHECK(cudaMemcpy(input_data_gpu, input_data.data(), SIZE * sizeof(float),
	...
	// Launch GPU kernel
	stencil_kernel_tiling_gpu<<<blocks_per_grid, threads_per_block>>>(input_data_gpu,
	                                                                  output_data_gpu, DIM);
	CHECK_KERNELCALL();
	...
	CHECK(cudaMemcpy(gpu_result.data(), output_data_gpu, SIZE * sizeof(float),
	                 cudaMemcpyDeviceToHost));
	end                  = get_time();
	long long elapsedGPU = end - start;
