...
typedef struct _HashMap {
    u32     size, cap;  // .hm.cap is a power of 2
    Bucket *buckets;
...
u32 shelf_lazy_update(Shelf *shl) {
    u32 retval = 0;
...
u32 test_1(VecQ w, VecQ *v, u32 i, u32 j) {
    if (w.inner[i].time < 0x80000000) {
        if (can_cook(v->inner[i])) {
            cook(w.inner[i]);
...
void test_2(VecQ w, VecQ *v, u32 i) {
    if (w.inner[i].time < 0x80000000 && can_cook(w.inner[i])) {
        cook(w.inner[i]);
...
void wq_check(VecQ *v) {
    ...
    if (v->del << CHECK_SHIFT > v->size) {  // @perf: either of the 2 alternatives can be cut
        u32 j = 0;
        for (u32 i = 0; i < w.size; i++) j = test_1(w, v, i, j);
        v->size = j;
        v->del  = 0;
    } else {
        for (u32 i = 0; i < w.size; i++) test_2(w, v, i);
    }
}

void skip_string() {
    for (last_char = getchar(); last_char != ' ' && last_char != '\n'; last_char = getchar());
}
...
u64 read_string_and_hash(char prev[][WORD_LEN]) {  // @hope no collision
    ...
    for (last_char = getchar(); last_char != ' ' && last_char != '\n'; last_char = getchar()) {
        ...
        hash = ((hash * P) + last_char - 47);  // hash_map.cap is a power of 2, but `%M` is useless
    }
    if (prev) prev[0][i++] = '\0';
...
void parse_input_and_calc() {
    ...
    for (u32 i = 0; i < 2; i++) last_char = getchar();
    if (last_char == 'g') {
        for (u32 i = 0; i < 14; i++) last_char = getchar();
        parse_recipe();
    } else if (last_char == 'd') {
        for (u32 i = 0; i < 4; i++) last_char = getchar();
        parse_order();
    } else if (last_char == 'm') {
        for (u32 i = 0; i < 13; i++) last_char = getchar();
        parse_remove();
    } else if (last_char == 'f') {
        for (u32 i = 0; i < 10; i++) last_char = getchar();
        parse_supply();
    } else assert(0);
}
