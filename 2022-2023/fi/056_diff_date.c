...
int giorniFineAnno(data_T *day) {
  ...
  for (i = day->mese + 1; i <= MESE_ANNO; i++) count += dayInMonth(i, day->anno);
...
int giorniInizioAnno(data_T *day) {
  ...
  count = day->giorno;
  for (i = day->mese - 1; i > 0; i--) count += dayInMonth(i, day->anno);
...
int contaGiorniAnni(int anno1, int anno2) {
  ...
  anno1_100 = anno1 % 100;
  anno1_400 = anno1 % 400;
  if (anno1_4 == 0 || anno1_4 + diff % 4 >= 4) count++;
  if (anno1_100 == 0 || anno1_100 + diff % 100 >= 100) count--;
  if (anno1_400 == 0 || anno1_400 + diff % 400 >= 400) count++;
