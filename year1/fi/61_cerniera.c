/*
La cerniera è uno schema enigmistico che segue una formula del tipo ZX / YZ = XY, ossia date due stringhe s1 e s2 cerca una sottostringa Z presente in entrambe, in s1 all’inizio, in s2 alla fine, e se esiste crea una nuova stringa concatenando ciò che avanza da s1 e s2. Per esempio, se s1
...
(2 pti) Scrivere un programma che acquisisce da riga di comando due stringhe e calcola, se esiste, la cerniera visualizzandola (chiamando i sottoprogrammi di cui sono state indicate le funzionalità precedentemente). Nel caso in cui la cerniera non esista, il programma non visualizza
nulla.
 */
